...
export default function ContentV2({ open }) {
  ...
  const skills = [
    [
      {symbol:<SiNextdotjs size={40} />, name:'Next.JS',detail:'App dir, Routing, Layouts, LoadingUI, and API routes'},
      {symbol:<FaReact size={40} />, name:'React.JS',detail:'Server and Client Component. Use hook'},
      {symbol:<FaDatabase size={40} />, name:'Database & ORM',detail:'ORM using Sequelize, Prisma. Postgresql and Mysql as databases'}
    ],
    [
      {symbol:<SiTailwindcss size={40} />, name:'Components',detail:'UI components built using Radix UI and styled with tailwind CSS'},
      {symbol:<FaShieldAlt size={40} />, name:'Authentication',detail:'Authentication using NextAuth.js or JWT and middlewares.'},
      {symbol:<RiFirebaseFill size={40} />, name:'Integration  & Event Systems',detail:'Using Rabbit MQ, Firebase, Socket.io '},
    ]]
  ...
  function tipBorder(input) {
    ...
    return input ? (
      <>
        <div class={`${sameClass} -top-2 left-0 transform -translate-x-1/2`}></div>
        <div class={`${sameClass} -bottom-2 right-0 transform translate-x-1/2`}></div>
        <div class={`${sameClass} -top-2 right-0 transform translate-x-1/2`}></div>
        <div class={`${sameClass} -bottom-2 left-0 transform -translate-x-1/2`}></div>
      </>
    ) : (
      <>
        <div class={`${sameClass} -bottom-2 right-0 transform translate-x-1/2`}></div>
    ...
    <div className="w-full h-full flex flex-col py-40">
      ...
      {generateSameDiv(
            <div className="w-full min-h-[600px] p-10 flex flex-col gap-10 ">
              ...
              <div className={`flex justify-start w-full h-fit gap-5`}>
                {listButtonProject.map((data, index) => (
                  <button
                    ...
                    className={`${index === title? "opacity-100 border-b border-orange-300": "opacity-50 hover:bg-orange-300 hover:opacity-100 hover:text-black"} p-3 rounded-xl`}
                  >
                    {data}
                  </button>
                ))}
              ...
              <div>
                {open.body3 ? (
                  <div className={`flex flex-col ${ feature.border ? "max-h-[1000px]" : "max-h-0 " } px-10 overflow-hidden border border-orange-300 rounded-lg duration-1000 ease-in-out`}>
...
{generateSameDiv(
        <div className="flex flex-col gap-10 items-center">
          <div className="flex flex-col gap-5 mt-10 px-8 py-6">
              <span className="text-center text-5xl font-semibold text-[">FRAMEWORK & TECH</span>
              <div className="flex flex-col items-center text-gray-500 text-lg">
              <span className="w-3/4">These are the tools and frameworks with which I feel more comfortable without limiting myself to them because i have worked with React, MySql, Express, Javascript and many more...</span>
          ...
          <div className="flex flex-col gap-5 mt-5 pb-10">
            ...
            skills.map(data=>{
               return <div className="flex justify-center gap-5 w-full">
                  ...
                  data.map(inside=>{
                    return <div
                     className="w-[30%] font-medium flex flex-col border border-gray-800 rounded-lg p-5">
                      <div>{inside.symbol}</div>
                      <div className="mt-4">{inside.name}</div>
                      <div className="text-gray-500 text-sm">{inside.detail}</div>
                  </div>
                  })
                  }
               </div>
